...
/**
 * @description Custom hook for managing chat state, interactions, and translations
 * @param {string} currentLang - Current language code for translation purposes
 * @returns {Object} Chat state and handlers: { messages, isLoading, sendMessage, clearChat }
 */
export function useChat(currentLang) {
  ...
  const sendMessageFunc = useCallback(
    async rawText => {
  ...
  return { messages, isLoading, sendMessage: sendMessageFunc, clearChat };
}
