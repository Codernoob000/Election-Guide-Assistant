...
export default function LanguageSelector({ className }) {
  ...
  return (
    <div className={`language-selector ${className || ''}`}>
      ...
      {isOpen && (
        <>
          <div className="language-selector__backdrop" onClick={() => setIsOpen(false)} />
          <ul className="language-selector__dropdown" role="listbox" aria-label={t('language.select')}>
            {SUPPORTED_LANGUAGES.map((lang) => (
              <li key={lang.code} role="option" aria-selected={lang.code === i18n.language}>
                ...
                >
                  ...
                  {lang.code === i18n.language && <span className="language-selector__check">✓</span>}
                </button>
              </li>
            ))}
          </ul>
